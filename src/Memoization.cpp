...
#define USE_FULL_NAMES_IN_KEY 0
    ...
    Stmt call_copy_memory(const std::string &key_name, const std::string &value, Expr index) {
        Expr dest = AddressOf::make(Handle(), key_name, index, Buffer<>(), Parameter());
...
public:
  KeyInfo(const Function &function, const std::string &name)
    ...
    Expr generate_lookup(std::string key_allocation_name, std::string computed_bounds_name,
        ...
        args.push_back(AddressOf::make(type_of<uint8_t *>(), key_allocation_name, {Expr(0)}, Buffer<>(), Parameter()));
        ...
        if (!innermost_realization_name.empty() &&
            ...
            if (arg1 != nullptr) {
                internal_assert(arg1->args.size() == 1 && !arg1->func.defined())
                    << "RewriteMemoizedAllocations: AddressOf should be to a load.\n";

                const IntImm *index = arg1->args[0].as<IntImm>();

                if (index != nullptr && index->value == 0 &&
                    get_realization_name(arg1->name) == innermost_realization_name) {
