...
Stmt inject_profiling(Stmt s, string pipeline_name) {
    ...
    Expr func_names_buf = AddressOf::make(Handle(), "profiling_func_names", {0}, Buffer<>(), Parameter());
    ...
    if (!no_stack_alloc) {
        Expr func_stack_peak_buf = AddressOf::make(Handle(), "profiling_func_stack_peak_buf", {0}, Buffer<>(), Parameter());
